    ...
    const fetchTemplate = async () => {
      ...
      try {
        const [templateRes, templatesRes] = await Promise.all([
          axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/templates/${slug}`),
          axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/templates`),
        ]);
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_0.8fr] gap-10 items-start">
                <div>
                  <div className="flex flex-wrap items-center gap-3 mb-5">
                    <span className="inline-flex items-center gap-2 px-3 py-1 bg-brand-purple/20 text-brand-cyan text-sm rounded-full">
                      <Layers3 size={14} />
                      {template.category}
                    </span>
                  ...
                  <div className="flex flex-wrap gap-3 mt-8">
                    {template.tags?.map((tag) => (
                      <span
                        key={tag}
                        className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full text-sm text-slate-300"
                      >
                        <Tag size={14} />
                        {tag}
                      </span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-8">
              ...
              <div className="card-glass">
                ...
                <h2 className="font-syne font-bold text-3xl text-white mb-6">
                  Fast Launches Without Sacrificing Presentation
                </h2>
                <p className="text-slate-300 text-lg leading-relaxed mb-8">
                  This store item is a strong fit for teams that want an elegant public-facing presence without starting
                  every section from scratch. It keeps layout, messaging hierarchy, and visual rhythm consistent across
                  desktop and mobile.
                </p>
                <div className="flex flex-wrap gap-2 mb-8">
                  {(template.useCases || []).map((item) => (
                    <span key={item} className="px-3 py-1.5 bg-white/5 rounded-full text-sm text-slate-300">
                      {item}
                    </span>
                  ))}
                </div>
