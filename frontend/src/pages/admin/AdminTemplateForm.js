...
const AdminTemplateForm = () => {
    ...
    if (isEdit) {
      const fetchTemplate = async () => {
        try {
          ...
          const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/api/templates?published_only=false`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/templates/${id}`, data, { headers });
        toast.success('Store item updated successfully');
      } else {
        await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/templates`, data, { headers });
