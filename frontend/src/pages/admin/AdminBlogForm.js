...
const AdminBlogForm = () => {
    ...
    if (isEdit) {
      const fetchBlog = async () => {
        try {
          ...
          const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/blogs?published_only=false`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put(`${process.env.REACT_APP_BACKEND_URL}/api/blogs/${id}`, data, { headers });
        toast.success('Blog updated successfully');
      } else {
        await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/blogs`, data, { headers });
