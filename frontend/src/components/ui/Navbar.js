...
const Navbar = () => {
  ...
  const isNavActive = (path) => path === '/services' ? location.pathname.startsWith('/services') : isActive(path);
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`font-dm-sans text-sm font-medium transition-colors relative ${
                  isNavActive(link.path) ? 'text-brand-cyan' : 'text-slate-300 hover:text-white'
                }`}
              >
                {link.name}
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`block font-dm-sans text-lg font-medium py-2 ${
                    isNavActive(link.path) ? 'text-brand-cyan' : 'text-slate-300'
                  }`}
                >
                  {link.name}
                </Link>
